...
namespace DS
{
	...
	public:
		...
		//Precondition:
		//user provided interest rate as double
		//Postcondition:
		//account's existing interest rate is updated
		const void setAPR(double interestRate);

		//Precondition:
		//None
		//Postcondition:
		//account's existing current balance is returned as double
		const double getCurrentBalance() const;

		//Precondition:
		//user provides a day as an int
		//Postcondition:
		//account's balance on user given day
		const double getPreviousBalance(size_t inputDay) const;
	...
	private:
		...
		size_t numOfDays;
		//Creates dailyBalance ptr
		double *dailyBalanceptr;
	};
}
#endif;